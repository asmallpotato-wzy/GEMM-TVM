...
packedA_pre = te.compute(
    (M//mr,K,mr), lambda mo,k,mi: A[mo*mr+mi,k],
    name = 'packedA_pre'
)
s2 = te.create_schedule(packedA_pre.op)
packA_func = tvm.build(s2, [A,packedA_pre], target=target, name='fpackedA')
assert  packA_func
packed_a = tvm.nd.array(numpy.zeros((M//mr,K,mr), dtype=dtype), dev)
packA_func(a, packed_a)
packedA = te.placeholder((M//mr,K,mr),name= "packedA")
...
packedC = te.compute(
    (N//nc,M,nc), lambda no,m,ni:te.sum(packedA[m//mr,k,tvm.tir.indexmod(m,mr)]*packedB[(no*nc+ni)//nr,k,tvm.tir.indexmod(no*nc+ni,nr)],axis = k),
    name = 'packedC'
)
...
func = tvm.build(s, [packedA, packedB, C], target=target, name='mmult')
assert func
#print(func.get_source("asm"))
print(tvm.lower(s, [packedA, packedB, C], simple_mode=True))

c = tvm.nd.array(numpy.zeros((M,N), dtype=dtype), dev)
func(packed_a, packed_b, c)
tvm.testing.assert_allclose(c.numpy(), answer, rtol=1e-5)

evaluator = func.time_evaluator(func.entry_name, dev, number=10,repeat=10)
mean_time = evaluator(packed_a, packed_b, c).mean
#print(c.numpy())
